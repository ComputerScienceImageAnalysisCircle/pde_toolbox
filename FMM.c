...
)
{
	...
	heapStruct->log2HeapSize = (int)ceil(log(num_pixels + 1.0f)/log(2.0f));
	...
	BIMAGE *phi)				/* The distance function */
{
	...
	/* Now shuffle thisa point up the heap to where it belongs */
	...
	BIMAGE *phi) 			/* The 'distance' function by which we sort */
{
	...
	/* Shuffle thisa point to where it belongs */
	if (new_phi < old_phi) {
		heapShuffleUp(heapStruct, index, phi);
	} else {
...
RIE_INT * RIE_INT_constructor(
	...
	thisa = (RIE_INT *)malloc(sizeof(RIE_INT));
...
RIE_FLOAT * RIE_FLOAT_constructor(
	...
	thisa = (RIE_FLOAT *)malloc(sizeof(RIE_FLOAT));
...
RIE_SURF * RIE_SURF_constructor(
	...
	thisa = (RIE_SURF *)malloc(sizeof(RIE_SURF));
...
int RIE_INT_add(
	...
	newa.x = thisa->x + dx;
	newa.y = thisa->y + dy;
	newa.z = thisa->z;			/* Default value */

	/* Check if we have to change layer */
	/* Simplify things by an approximation */
	if (newa.x < rie_surf->source_x - rie_surf->source_radius
		&& thisa->x < rie_surf->source_x - rie_surf->source_radius) {
		if (newa.y >= rie_surf->source_y && thisa->y < rie_surf->source_y) {
			newa.z = thisa->z + 1;
		} else if (newa.y < rie_surf->source_y && thisa->y >= rie_surf->source_y) {
			newa.z = thisa->z - 1;
		} else {
			newa.z = thisa->z;
		}
	...
	if (restrict_to_surface)
		if (!RIE_INT_on_surface(&newa, rie_surf))
			return 1;

	/* Copy temporary coordinate to output */
	memcpy(thisa, &newa, sizeof(RIE_INT));
...
int RIE_FLOAT_add(
	float dx, float dy,
	char restrict_to_surface,		/* Restrict the newa point to the Riemann surface? */
	RIE_SURF * rie_surf,
	RIE_FLOAT * thisa
) {
	RIE_FLOAT newa;
	...
	/* Check if we have to change layer */
	/* Simplify things by an approximation */
	if (newa.x < rie_surf->source_x && thisa->x < rie_surf->source_x) {
		if (newa.y >= rie_surf->source_y && thisa->y < rie_surf->source_y) {
			newa.z = thisa->z + 1;
		} else if (newa.y < rie_surf->source_y && thisa->y >= rie_surf->source_y) {
			newa.z = thisa->z - 1;
	...
	/* Restrict the coordinate to a valid surface point */
	if (restrict_to_surface)
		if (!RIE_FLOAT_on_surface(&newa, rie_surf))
			return 1;

	/* Copy temporary coordinate to output */
	memcpy(thisa, &newa, sizeof(RIE_FLOAT));
...
int RIE_INT_to_index(
	...
	index = thisa->x + rie_surf->nx*(thisa->y + rie_surf->ny*thisa->z);
...
char RIE_INT_on_surface(
	...
	/* Check outer bounds */
	if (thisa->x < 0 || thisa->x > rie_surf->nx - 1) {
		return LSTB_FALSE;
	}
	if (thisa->y < 0 || thisa->y > rie_surf->ny - 1) {
		return LSTB_FALSE;
	}
	if (thisa->z < 0 || thisa->z > rie_surf->num_revs - 1) {
		return LSTB_FALSE;
	}

	/* Check it doesn't clash with the seed point */
	if (LSTB_ABS(thisa->x - rie_surf->source_x) <= rie_surf->source_radius
		&& LSTB_ABS(thisa->y - rie_surf->source_y) <= rie_surf->source_radius) {
		return LSTB_FALSE;
	}
