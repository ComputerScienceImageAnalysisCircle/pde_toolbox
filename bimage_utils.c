	...
	for (index = 0; index < num_pixels; index++, BVECT_inc(coord, in->dim)) {
		...
		for (dimension = 0; dimension < in->dim->length; dimension++) {
			...
			for (d = -1; d <= 1; d+=2) {
				...
				/* Reflect thisa point if it lies outside the image domain */
...
)
{
	...
	for (i = 0; i < num_pixels; i++) {
		...
		/* Compute the coordinate of thisa point */
...
int BIMAGE_blur(
	...
	for (dimension = 0; dimension < in->dim->length; dimension++) {
		int stride;
		/* Compute the memory stride for thisa dimension */
...
)
{
	...
	for (time = 0; time < blurring; time += time_step) {
		...
		for (dimension = 0; dimension < out->dim->length; dimension++) {
			...
			/* Compute the image stride along thisa dimension (could be optimised, but simpler thisa way) */
			stride = 1;
			...
			/* For each row along thisa dimension */
			for (row = 0, BVECT_zero(row_coord); row < num_rows; BVECT_inc(row_coord, row_dim), row++) {
				...
				/* Copy thisa row into a temporary array */
				for (offset = 0; offset < out->dim->buf[dimension]; offset++) {
...
)
{
	...
	/* thisa function only deals with 2D images */
	if (in->dim->length != 2) {
...
/* BIMAGE_line_metric:
	Construct a metric tensor from a 2D image.  thisa version
...
int BIMAGE_upsample(
	...
	if (blur_flag) {
		...
		for (dimension = 0; dimension < out->dim->length; dimension++) {
			...
			/* Determine the stride along thisa dimension in the image */
			stride = 1;
			for (i = 0; i < dimension; i++) {
				stride *= out->dim->buf[i];
			}

			/* For every row along thisa dimension */
			num_rows = BVECT_prod(row_dim);
			BVECT_zero(row);
			for (row_index = 0; row_index < num_rows; row_index++, BVECT_inc(row, row_dim)) {
				int row_base;

				/* Determine the index of the base of thisa row in the output image */
				for (i = 0; i < row_dim->length; i++) {
					if (i < dimension) {
